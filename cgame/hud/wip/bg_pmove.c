...
PM_CorrectAllSolid
=============*/
static qboolean PM_CorrectAllSolid(pmove_t* pm, playerState_t* ps, pml_t* pml, trace_t* trace)
{
  vec3_t point;
...
void hud_GroundTrace(pmove_t* pm, playerState_t* ps, pml_t* pml)
{
  ...
  if (trace.allsolid)
  {
    if (!PM_CorrectAllSolid(pm, ps, pml, &trace)) return;
  }
...
/*
=============
hud_SetWaterLevel  FIXME: avoid this twice?  certainly if not moving
=============*/
...
Sets mins, maxs, and ps.viewheight
==============*/

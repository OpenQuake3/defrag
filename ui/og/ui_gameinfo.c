...
static void UI_LoadArenas(void) {
  ...
  // set initial numbers
  for (n = 0; n < ui_numArenas; n++) { Info_SetValueForKey(ui_arenaInfos[n], "num", va("%i", n)); }
...
const char* UI_GetArenaInfoByNumber(int num) {
  ...
  for (n = 0; n < ui_numArenas; n++) {
    ...
    if (*value && atoi(value) == num) { return ui_arenaInfos[n]; }
  }
...
const char* UI_GetArenaInfoByMap(const char* map) {
  ...
  for (n = 0; n < ui_numArenas; n++) {
    if (Q_stricmp(Info_ValueForKey(ui_arenaInfos[n], "map"), map) == 0) { return ui_arenaInfos[n]; }
  }
...
const char* UI_GetSpecialArenaInfo(const char* tag) {
  ...
  for (n = 0; n < ui_numArenas; n++) {
    if (Q_stricmp(Info_ValueForKey(ui_arenaInfos[n], "special"), tag) == 0) { return ui_arenaInfos[n]; }
  }
...
char* UI_GetBotInfoByName(const char* name) {
  ...
  for (n = 0; n < ui_numBots; n++) {
    ...
    if (!Q_stricmp(value, name)) { return ui_botInfos[n]; }
  }
...
void UI_GetBestScore(int level, int* score, int* skill) {
  ...
  if (!score || !skill) { return; }

  if (level < 0 || level > ui_numArenas) { return; }
  ...
  for (n = 1; n <= 5; n++) {
    ...
    if (skillScore < 1 || skillScore > 8) { continue; }
...
void UI_SetBestScore(int level, int score) {
  ...
  if (oldScore && oldScore <= score) { return; }
...
int UI_TierCompleted(int levelWon) {
  ...
  int tier  = levelWon / ARENAS_PER_TIER;
  int level = tier * ARENAS_PER_TIER;

  if (tier == UI_GetNumSPTiers()) {
    info = UI_GetSpecialArenaInfo("training");
    if (levelWon == atoi(Info_ValueForKey(info, "num"))) { return 0; }
    info = UI_GetSpecialArenaInfo("final");
    if (!info || levelWon == atoi(Info_ValueForKey(info, "num"))) { return tier + 1; }
    return -1;
  }
...
qboolean UI_CanShowTierVideo(int tier) {
  ...
  if (!tier) { return qfalse; }

  if (uis.demoversion && tier != 8) { return qfalse; }

  trap_Cvar_VariableStringBuffer("g_spVideos", videos, sizeof(videos));

  Com_sprintf(key, sizeof(key), "tier%i", tier);
  if (atoi(Info_ValueForKey(videos, key))) { return qtrue; }
...
===============
*/
int UI_GetNumArenas(void) { return ui_numArenas; }
...
===============
*/
int UI_GetNumSPArenas(void) { return ui_numSinglePlayerArenas; }
...
===============
*/
int UI_GetNumSPTiers(void) { return ui_numSinglePlayerArenas / ARENAS_PER_TIER; }
...
void UI_SPUnlock_f(void) {
  ...
  // unlock cinematics
  for (tier = 1; tier <= 8; tier++) { UI_ShowTierVideo(tier); }
